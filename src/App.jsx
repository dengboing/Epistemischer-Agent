...
// API Call über Vercel Serverless Function (kein CORS-Problem)
...
export default function App() {
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      if (useLive) {
        // Live API-Modus via Serverless Function
        const apiMessages = newMessages.map(m => ({
          role: m.role,
          content: m.text,
        }));
        assistantText = await callClaude(apiMessages);
      } else {
        // Demo-Modus
  ...
  }, [input, loading, messages, mode, step, useLive]);
  ...
  if (screen === "welcome") return (
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem", fontFamily: "'Georgia', serif" }}>
      <div style={{ maxWidth: "540px", width: "100%", textAlign: "center" }}>
        ...
        {/* Live-Modus Indikator */}
        <div style={{ fontSize: "0.7rem", color: C.textMuted, marginBottom: "1.5rem" }}>
          🟢 Live-Modus
        </div>
    ...
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", flexDirection: "column", fontFamily: "'Georgia', serif" }}>
      ...
      <div style={{ padding: "1rem 1.25rem", borderTop: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        ...
        <div style={{ fontSize: "0.65rem", color: C.textMuted, marginTop: "0.5rem" }}>
          Enter senden · Shift+Enter neue Zeile · 🟢 Live
        </div>
      </div>
